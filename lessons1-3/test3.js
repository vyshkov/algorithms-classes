...
//nlog(n)
function binarySearchRec(arr = [], target, a = 0, b = arr.length - 1) { // log(n)
  if (a > b) {  // O(1)
      return -1; // O(1)
  }
...
function binarySearch(arr = [], target) { // O(log (n)), O(1)
    let a = 0;
    let b = arr.length - 1;
...
function findRotatedIndex(arr = [3, 4, 5, 6, 7, 1, 2]) {  // log(n), O(1)
    let a = 0;
    let b = arr.length - 1;
    let prev = arr[0];
