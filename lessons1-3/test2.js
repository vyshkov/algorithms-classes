...
function is100hDoorOpened(n) { // time O(n ^ 2), space O(n)
    let doors = new Array(n + 1).fill(false); // O(n)

    for (let i = 1; i < doors.length; i++) { // O(n)
        for (var j = 1; j < doors.length; j++) { // O(n)
            if (j % i === 0) {
                doors[j] = !doors[j];
            }
...
function is100hDoorOpened2(n) { // O(1)
...
function containsAll(arr1 = [1, 2, 2, 3], arr2 = [2,1,6,3]) { // n log n, O(n)
...
function findPair(arr = [1,2,4,10], x = 6) { // O(n^2)
...
// 1, 2, 4, 10
//    i  j
function findPair2(arr = [1,2,3,4,10], x = 7) { // O(n^2)
    let i = 0;
    let j = arr.length - 1;
    let sum;
